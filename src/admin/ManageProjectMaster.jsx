...
const ManageProjectMaster = () => {
  ...
  const [projects, setProjects] = useState([
    { projectID: 1, projectTitle: 'Student Project Management System', description: 'A role-based Web API and React system for managing academic project allocations, task tracking, evaluations, and progress monitoring.' },
    { projectID: 2, projectTitle: 'E-Commerce Engine', description: 'A high-throughput API gateway and storefront backend with inventory tracking, Redis caching, and payment integration.' },
    { projectID: 3, projectTitle: 'IoT Smart Home', description: 'A real-time home automation dashboard using MQTT protocol, React, and Raspberry Pi sensors integration.' }
  ]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (modalType === 'Add') {
      const newId = projects.length > 0 ? Math.max(...projects.map(pr => pr.projectID)) + 1 : 1;
      setProjects([...projects, { ...formData, projectID: newId }]);
    } else if (modalType === 'Edit' && selectedProject) {
      setProjects(projects.map(pr => pr.projectID === selectedProject.projectID ? { ...pr, ...formData } : pr));
    }
    handleCloseModal();
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this Project Master record?')) {
      setProjects(projects.filter(pr => pr.projectID !== id));
    }
  };
